...
def inputing(case):
    ...
    embeded=emb.multi_d_embedding(inp,n_tau=17, tau=1)
...
for root, dirs, files in os.walk(pth):
    for fname in files:
        if fname.startswith(('SHM','LID','LES')) and fname.endswith('csv') :
...
trk_p=10
forecast_l=5300
saving_path='/home/morteza/dlc_projects/Analysis/Currencodes/TDA/simplex_proj/12D/'

for i in range(len(fname_list)):
    
    manif=inputing(csv_list[i])
    low_tp=int(0.65*len(manif))
    high_tp=int(0.8*len(manif))

    samps=np.random.randint(low_tp, high_tp, n_samps, dtype='int')

    output=prediction_scores(fname_list[i], manif, dim, samps,forecast_l, trk_p)
    df=pd.DataFrame(output, columns=['treatment', 'animal', 'smapling_point', 'dimension', 'prediction_length', 'mean_score'])
    df.to_csv(saving_path+'12D_simplex_'+fname_list[i][:9]+'.csv')
    print(saving_path+'simplex_'+fname_list[i][:9]+'.csv')
