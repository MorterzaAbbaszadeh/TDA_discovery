...
for root, dirs, files in os.walk(pth):
    for fname in files:
        if not fname.startswith(('SHM', 'LES')) and fname.endswith('csv') :
...
for i in range(len(fname_list)):
    ...
    cause=get_head(csv_list[i], dim)[::10] #downsampling is essential

    
    targt=get_body(csv_list[i], dim)[::10]

    output_scr=get_ccms(cause, targt, dims, forecast_l)

    for j in range(len(output_scr)):
        df_list.append((fname_list[i][:3], output_scr[j], j))  #animal no, treatment, score, dimention.


df=pd.DataFrame(df_list, columns=['treatment', 'score', 'dimention'])
df.to_csv(saving_path+'CCM_bodytohead_treatment.csv')