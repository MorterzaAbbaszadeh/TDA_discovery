...
def inputing(case):
    ...
    embeded=emb.multi_d_embedding(inp,n_tau=50, tau=1)
...
for root, dirs, files in os.walk(pth):
    for fname in files:
        if fname.startswith('SHM') and fname.endswith('csv') :
...
trk_p=10
forecast_l=300
saving_path='/home/morteza/dlc_projects/Analysis/Currencodes/TDA/'
...
for i in range(len(fname_list)):
    ...
    df.to_csv(saving_path+'comb_simplex_'+fname_list[i][:9]+'.csv')
    print(saving_path+'comb_simplex_'+fname_list[i][:9]+'.csv')
