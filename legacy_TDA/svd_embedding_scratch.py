...
for root, dirs, files in os.walk(pth):
    for fname in files:
        ...
        if fname.endswith('.csv') and fname[:3] in (['SUM', 'LID', 'SKF', 'D1A', 'D2A']):
...
dframe_t=dframe[dframe['fname'].isin([ 'LID', 'SKF', 'SUM', 'D2A'])]
...
plt.ylabel('Log(sigma)', fontsize='large')
...
sns.boxplot(x='fname', y='delta', data=dframe.loc[dframe['tau'].isin([47])], ax=ax1,
        order=['SKF', 'D1A', 'SUM', 'LID'], showfliers=False)



plt.ylabel('Predicted Length', fontsize='large')
plt.xlabel('')
plt.text(0.5,-7.5,'Dimension 47', fontsize='large')
plt.xlim(-0.7,5)
sns.despine()
...
#%%
case='/home/morteza/dlc_projects/Analysis/Currencodes/TDA/Visualization/pilot_data/LID28.csv'
...
case='/home/morteza/dlc_projects/Analysis/Currencodes/TDA/Visualization/pilot_data/LID27.csv'
...
train_frac=0.88 #lowering reduces accuracy
forecast_l=300
